...
namespace Frertex
{
	...
	enum class ETypeIDs : std::uint64_t
	{
		...
		BuiltinEnd = 0x8000'0000'0000'0000
	};
	...
	{
	public:
		FunctionDeclaration(Utils::CopyMovable<std::string>&& name, EEntrypointType type, Utils::CopyMovable<std::vector<FunctionParameter>>&& parameters, Utils::CopyMovable<std::vector<LabelInfo>>&& labels, Utils::CopyMovable<std::vector<LabelRefInfo>>&& labelRefs, Utils::CopyMovable<std::vector<std::uint32_t>>&& code, Utils::CopyMovable<std::vector<LineInfo>>&& lines)
		    : FunctionDefinition(name.get(), type), m_Parameters(parameters.get()), m_Labels(labels.get()), m_LabelRefs(labelRefs.get()), m_Code(code.get()), m_Lines(lines.get()) {}
	...
	{
	public:
		FILEntrypoint(EEntrypointType type, std::uint64_t labelID, Utils::CopyMovable<std::vector<FILEntrypointParameter>>&& inputs, Utils::CopyMovable<std::vector<FILEntrypointParameter>>&& outputs)
		    : m_Type(type), m_LabelID(labelID), m_Inputs(inputs.get()), m_Outputs(outputs.get()) {}

	public:
		EEntrypointType m_Type;
		std::uint64_t   m_LabelID;

		std::vector<FILEntrypointParameter> m_Inputs;
		std::vector<FILEntrypointParameter> m_Outputs;
	};

	struct FILFunctionParameter
	{
	public:
		std::vector<ETypeQualifier> m_Qualifiers;
	...
	{
	public:
		FILFunction(std::uint64_t labelID, std::uint64_t returnTypeID, Utils::CopyMovable<std::vector<FILFunctionParameter>>&& parameters)
		    : m_LabelID(labelID), m_ReturnTypeID(returnTypeID), m_Parameters(parameters.get()) {}

	public:
		std::uint64_t m_LabelID;

		std::uint64_t                     m_ReturnTypeID;
		std::vector<FILFunctionParameter> m_Parameters;
	};
	...
	public:
		...
		std::vector<FILFunction>   m_Functions;
		std::vector<std::uint8_t>  m_Strings;
		std::vector<FILType>       m_Types;
